...
func _load_database() -> void:
    ...
    for team in database.get("teams", []):
        teams_by_id[str(int(team.get("id", -1)))] = team
        club_squads[str(int(team.get("id", -1)))] = team.get("players", []).duplicate(true)
...
func _show_main_menu() -> void:
    ...
    var version = _label("Прототип v0.9.1 · Godot 4", 12, colors.muted)
...
func _show_new_game() -> void:
    ...
    root.add_child(_label("Выберите клуб, продолжительность карьеры и первого спонсора.", 14, colors.muted))

    var clubs_panel = PanelContainer.new()
    clubs_panel.add_theme_stylebox_override("panel", _panel_style(colors.panel, Color("19384b"), 12, 1))
    root.add_child(clubs_panel)
    var clubs_grid = GridContainer.new()
    clubs_grid.columns = 2
    clubs_grid.add_theme_constant_override("h_separation", 14)
    clubs_grid.add_theme_constant_override("v_separation", 14)
    clubs_panel.add_child(clubs_grid)

    for team in _playable_teams():
        var card = Button.new()
        card.toggle_mode = true
        card.button_group = _new_game_button_group()
        card.custom_minimum_size = Vector2(420, 100)
        card.text = "%s\nБюджет: %s   ·   Сила состава: %d" % [team.get("name", "Клуб"), _money(int(team.get("budget", 0))), _team_base_rating(int(team.get("id", -1)))]
        card.add_theme_font_size_override("font_size", 16)
        card.add_theme_stylebox_override("normal", _panel_style(colors.panel_2, Color("27475c"), 10, 1))
        card.add_theme_stylebox_override("hover", _panel_style(Color("15384d"), colors.cyan, 10, 1))
        card.add_theme_stylebox_override("pressed", _panel_style(Color("135167"), colors.mint, 10, 2))
        card.pressed.connect(_select_new_club.bind(int(team.get("id", -1)), card))
        clubs_grid.add_child(card)
    ...
    sponsor_box.add_child(_label("После каждого матча начисляются бонусы за результат. В конце сезона доступны отдельные премии за чемпионство и попадание в тройку.", 12, colors.muted))
...
func _show_dashboard(tab: String) -> void:
    ...
    var nav_items = [
        ["club", "СОСТАВ"], ["tactics", "ТАКТИКА"], ["match", "МАТЧ"],
...
func _render_tactics() -> void:
    ...
    bench_box.add_child(_label("Скамейка и резерв", 18, colors.text))
    ...
    var sorted_squad = _squad(selected_team_id).duplicate()
...
func _render_match() -> void:
    ...
    if current_match.is_empty():
        ...
        box.add_theme_constant_override("separation", 14)
        panel.add_child(box)
        box.add_child(_label("Следующий матч · Тур %d" % int(next.get("round", 1)), 14, colors.mint))
    ...
    match_info.custom_minimum_size = Vector2(285, 0)
    ...
    info_box.add_child(_label("Установки", 18, colors.text))
    info_box.add_child(_label("Схема: %s" % game_state.get("formation", "4-4-2"), 14, colors.cyan))
    info_box.add_child(_label("Стиль: %s" % game_state.get("tactical_style", "Сбалансированно"), 14, colors.cyan))
    info_box.add_child(_label("Сила состава: %d" % int(_user_team_power()), 14, colors.mint))
    info_box.add_child(_label("Замены: %d из %d" % [int(current_match.get("substitutions", 0)), MAX_SUBSTITUTIONS], 14, colors.text))
    info_box.add_child(_label("Модель результата:\n60% — сила состава\n30% — случайность\n10% — удача", 13, colors.muted))
...
func _simulate_team_segment(attacker_id: int, defender_id: int, start_minute: int, end_minute: int, is_home: bool) -> void:
    ...
    var strength_probability = clamp(0.5 + (attack_power - defense_power) / 112.0, 0.15, 0.85)
    var random_component = rng.randf()
...
func _finish_match() -> void:
    ...
    fixture["report"] = {"home_stats": current_match.get("home_stats", {}), "away_stats": current_match.get("away_stats", {})}
...
func _render_calendar() -> void:
    ...
    for fixture in fixtures:
        ...
        row.add_child(h)
        var round_label = _label("Тур %d" % int(fixture.get("round", 1)), 13, colors.mint if involves_user else colors.muted)
...
func _render_transfers() -> void:
    ...
    for player_id in game_state.get("market_ids", []):
        var player = _player(int(player_id))
...
func _buy_player(player_id: int, contract_years = 3) -> void:
    ...
    if owner_id >= 0:
        club_squads[str(owner_id)].erase(player_id)
...
func _ai_surplus_player(team_id: int, squad: Array) -> int:
    ...
    for raw_id in squad:
        var player = _player(int(raw_id))
        if player.is_empty() or _is_goalkeeper(player) or int(player.get("age", 20)) < 20:
            continue
        candidates.append(int(raw_id))
    candidates.sort_custom(func(a, b): return int(_player(a).get("rating", 0)) < int(_player(b).get("rating", 0)))
    return int(candidates[0]) if not candidates.is_empty() else -1
...
func _load_game() -> void:
    ...
    for key in saved_squads.keys():
        club_squads[str(key)] = saved_squads[key]
    ...
    for team in database.get("teams", []):
        ...
        for player_id in team.get("players", []):
            if int(player_id) not in squad:
                squad.append(int(player_id))
    ...
    _clean_transfer_list()
    _ensure_set_piece_assignments()
    notice_text = "Сохранение загружено и обновлено до версии v0.9.1."
...
func _generate_fixtures(team_ids: Array) -> Array:
        ...
        for i in range(int(n / 2)):
            ...
            if home >= 0 and away >= 0:
                if round_index % 2 == 1:
                    ...
                    home = away
                    away = temp
                first_half.append({"round": round_index + 1, "home": home, "away": away, "played": false, "home_score": 0, "away_score": 0})
        var fixed = teams[0]
    ...
    for fixture in first_half:
        result.append({"round": int(fixture.get("round", 1)) + second_round_start, "home": int(fixture.get("away", -1)), "away": int(fixture.get("home", -1)), "played": false, "home_score": 0, "away_score": 0})
...
func _simulate_other_fixtures(round_number: int, excluded_index: int) -> void:
    for i in range(fixtures.size()):
        ...
        if int(fixture.get("round", -1)) != round_number or bool(fixture.get("played", false)):
            continue
...
func _quick_goals(attack: float, defense: float, home: bool) -> int:
    var expectation = 1.15 + (attack - defense) / 45.0 + (0.18 if home else 0.0)
    expectation = clamp(expectation, 0.25, 2.6)
    var goals = 0
    for _chance in range(5):
        if rng.randf() < expectation / 5.0:
            goals += 1
    if rng.randf() < 0.04:
        goals += 1
    return min(goals, 6)
...
func _team_match_power(team_id: int) -> float:
    ...
    if team_id == selected_team_id:
        ...
        var starting_ids = _team_starting_ids(team_id)
        var total = 0.0
        for player_id in starting_ids:
            total += float(_player(int(player_id)).get("rating", 0))
...
func _team_starting_ids(team_id: int) -> Array:
    ...
    for player_id in _squad(team_id):
        var player = _player(int(player_id))
...
func _distribute_quick_goals(lineup_ids: Array, goals: int, team_id = -1) -> void:
    for _goal in range(goals):
        ...
        if rng.randf() < 0.76:
            ...
            _add_player_stat(int(assister.get("id", -1)), "assists", 1)
...
func _open_player_dialog(player_id: int) -> void:
    ...
    root.add_child(_label("Возраст: %d · рейтинг: %d · стоимость: %s" % [int(player.get("age", 0)), int(player.get("rating", 0)), _money(int(player.get("value", 0)))], 14, colors.text))
...
func _loan_player_out(player_id: int) -> void:
    if not bool(_transfer_window_status().get("open", false)):
...
func _return_loans() -> void:
    ...
    for key in loans.keys():
        ...
        if int(loan.get("return_season", 999)) <= int(game_state.get("season", 1)):
            var player_id = int(key)
            if player_id not in club_squads[str(selected_team_id)]:
...
func _sorted_statistics_for_comp(category: String, competition_id: String, goalkeepers_only = false) -> Array:
    ...
    for key in player_stats.keys():
        var pid = int(key); var owner = _club_for_player(pid)
        if owner < 0 or str(_team(owner).get("competition","")) != competition_id: continue
        var pl = _player(pid)
        if pl.is_empty() or bool(pl.get("retired",false)): continue
        if goalkeepers_only and not _is_goalkeeper(pl): continue
        rows.append({"player_id":pid,"stats":player_stats[key]})
    rows.sort_custom(func(a,b):
        var sa:Dictionary=a["stats"]; var sb:Dictionary=b["stats"]
        var va = _stat_numeric_value(category,sa); var vb = _stat_numeric_value(category,sb)
        if not is_equal_approx(va,vb): return va>vb
